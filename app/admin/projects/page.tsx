    ...
    <div className="min-h-screen bg-[#FAF9F5] font-sans pb-12">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between h-auto sm:h-20 py-4 sm:py-0 gap-4">
            ...
            <Link
              ...
              className="inline-flex items-center justify-center px-5 py-2.5 bg-slate-900 text-white rounded-none hover:bg-[#1C1917] text-sm font-medium transition-all shadow-sm w-full sm:w-auto"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            { title: 'Total Projects', value: projects.length, icon: Building, color: 'text-[#1C1917]', bg: 'bg-stone-50' },
            { title: 'Active Projects', value: projects.filter(p => p.isActive).length, icon: CheckCircle, color: 'text-green-600', bg: 'bg-stone-50' },
            { title: 'Hidden Projects', value: projects.filter(p => !p.isActive).length, icon: XCircle, color: 'text-red-600', bg: 'bg-red-50' },
            { title: 'Total Media Files', value: totalMedia, icon: ImageIcon, color: 'text-stone-600', bg: 'bg-stone-50' },
          ].map((stat, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-white rounded-none border border-gray-200 shadow-sm p-6 flex items-center gap-5">
              <div className={`p-4 rounded-none ${stat.bg}`}>
        ...
        {/* --- FILTERS & SEARCH --- */}
        <div className="bg-white rounded-none border border-gray-200 shadow-sm p-4 sm:p-6 mb-8 flex flex-col lg:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 border border-gray-200 rounded-none text-sm focus:ring-2 focus:ring-[#1C1917] focus:border-transparent outline-none bg-[#FAF9F5] focus:bg-white transition-all"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="relative">
              ...
              <select
                ...
                className="w-full sm:w-auto pl-9 pr-8 py-2.5 border border-gray-200 rounded-none text-sm focus:ring-2 focus:ring-[#1C1917] outline-none bg-[#FAF9F5] cursor-pointer appearance-none"
              >
        ...
        {filteredProjects.length === 0 ? (
          <div className="bg-white rounded-none border border-gray-200 shadow-sm p-12 text-center flex flex-col items-center">
            ...
            {!searchTerm && selectedCategory === 'all' && (
              <Link href="/admin/projects/add" className="inline-flex items-center px-6 py-3 bg-slate-900 text-white rounded-none hover:bg-[#1C1917] transition-colors font-medium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-8">
            {filteredProjects.map((project, index) => (
              <motion.div
                ...
                className="bg-white rounded-none border border-gray-200 shadow-sm overflow-hidden flex flex-col hover:shadow-lg hover:border-[#1C1917]/50 transition-all duration-300 group"
              >
                {/* Project Image Header */}
                <div className="relative h-56 overflow-hidden bg-[#FAF9F5]">
                  <img 
                  ...
                  <div className="absolute top-4 left-4 flex gap-2">
                    <span className="inline-flex items-center px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded bg-white text-gray-900 shadow-sm">
                      {project.category === 'residential' ? <Home className="w-3 h-3 mr-1 text-[#1C1917]" /> : <Building className="w-3 h-3 mr-1 text-[#1C1917]" />}
                      {project.category}
                    </span>
                  </div>
                  ...
                  <div className="absolute top-4 right-4">
                    <span className={`inline-flex items-center px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded shadow-sm ${
                      project.isActive ? 'bg-stone-500 text-white' : 'bg-red-500 text-white'
                    }`}>
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  {/* Meta Details */}
                  <div className="grid grid-cols-2 gap-4 text-xs text-gray-600 mb-6 bg-[#FAF9F5] p-4 rounded-none border border-gray-100">
                    <div className="flex items-start">
                      <MapPin className="w-4 h-4 mr-2 text-[#1C1917] shrink-0" />
                      <span className="truncate">{project.location}</span>
                    </div>
                    <div className="flex items-start">
                      <User className="w-4 h-4 mr-2 text-[#1C1917] shrink-0" />
                      <span className="truncate" title={project.client}>{project.client}</span>
                    </div>
                    <div className="flex items-center col-span-2">
                      <Calendar className="w-4 h-4 mr-2 text-[#1C1917] shrink-0" />
                  ...
                  <div className="mt-auto">
                    ...
                    {project.technologies && project.technologies.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-5">
                        {project.technologies.slice(0, 3).map((tech, idx) => (
                          <span key={idx} className="px-2 py-1 bg-[#FAF9F5] border border-gray-200 text-gray-600 text-[10px] uppercase font-semibold tracking-wider rounded">
                            {tech}
                          </span>
                        ))}
                        {project.technologies.length > 3 && (
                          <span className="px-2 py-1 bg-[#FAF9F5] border border-gray-200 text-gray-500 text-[10px] font-semibold rounded">
                            +{project.technologies.length - 3}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2 pt-4 border-t border-gray-100">
                      <Link
                        ...
                        target="_blank"
                        className="flex-1 flex items-center justify-center px-3 py-2 border border-gray-200 text-gray-700 rounded-none hover:bg-[#FAF9F5] hover:text-gray-900 transition-colors text-sm font-medium"
                      >
                        <Eye className="w-4 h-4 mr-1.5 text-gray-400" /> View
                      </Link>
                      <Link
                        href={`/admin/projects/${project._id}?mode=edit`}
                        className="flex-1 flex items-center justify-center px-3 py-2 bg-slate-900 text-white rounded-none hover:bg-[#1C1917] transition-colors text-sm font-medium"
                      >
                        <Edit2 className="w-4 h-4 mr-1.5 opacity-80" /> Edit
                      </Link>
                      <button
                        onClick={() => handleDelete(project._id)}
                        className="px-3 py-2 border border-red-200 text-red-600 rounded-none hover:bg-red-50 hover:border-red-300 transition-colors"
